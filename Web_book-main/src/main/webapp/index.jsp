...
<%
    ...
    try {
        ...
        throw new RuntimeException("Error initializing ProductDao: " + e.getMessage(), e);
    }

    CategoryDAO categoryDAO = null;
    try {
        categoryDAO = new CategoryDAO(JDBCConnect.getConnection());
    } catch (ClassNotFoundException | SQLException e) {
        throw new RuntimeException("Error initializing CategoryDAO: " + e.getMessage(), e);
    }

    List<Category> categories = categoryDAO.selectAllCategory();
    List<Product> products = pd.get8TopLoadProducts();

    List<Cart> cartArrayList = (List<Cart>) session.getAttribute("cart-list");
                ...
                <div id="content-index" class="row">
                    <%
                        if (!products.isEmpty()) {
                            ...
                            %>
                            <div class="col-home" onclick = window.location="http://localhost:9999/getSingleProduct?id=<%=p.getId()%>">
                                <div class="card">
